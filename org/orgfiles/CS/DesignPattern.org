...
** 概述
#+BEGIN_VERSE
将各个具体请求封装成命令对象，实现统一的Execute()接口，从而可以使用不同的命令对象
对客户进行参数化；对请求排队[fn:6]；记录请求日志。另外还支持可撤销操作。
#+END_VERSE
** 适用性
- Command模式是回调机制的一个面向对象的替代品。
- 可在不同的时刻，指定、排列或者执行请求。这些操作甚至可在不同的进程中进行(依赖于存储命令的媒介)
- 支持取消操作。执行前记录状态，取消时利用该状态。考虑电风扇的例子[fn:7]。也可以将每次执行的状态用队列形式保存，可实现逐步Undo或逐步Redo。
- 支持修改日志。当系统崩溃时，记录下的日志过程可以重新来一遍。方法：添加Store()和Load()接口，利用序列化和持久化的方法记录日志。
- 支持事务。一个ConcreteCommand实现一个事务，由于接口一致，可以用同一种方式调用所有的事务。 *方便*
** 结构
   #+CAPTION: 
   #+ATTR_HTML: align="center"
   [[file:../../resources/OO/img/Command.png]]
** 参与者
- Client 创建一个具体命令对象并指定它的接收者。
- Invoker 存储ConcreteCommand(可提供SetCommand方法)，并在适当的时候调用Execute()
** 优点
1) 将调用命令的对象与知道如何实现该命令相关操作的对象解耦。
2) Command对象可像其他的对象一样被操纵和扩展。
** 实现
- 一个命令对象职责可大可小。
...
*** 如果原有的接口类可以修改
#+BEGIN_VERSE
如果允许直接修改用户期望接口或者Adaptee类的接口，
那么可以通过以下方式实现可插入式的适配器：
#+END_VERSE

1) 直接将Target与Client整合，Target接口直接变为Client的抽象方法
2) Client使用代理对象[fn:13]
...
- [[Mediator]]模式与Facade模式的相似之处：都抽象了一些已有的类的功能。但[[Mediator]]的目的是对同级之间的任意通讯进行抽象。[fn:8]
...
[fn:7] 风量大小为调速命令的状态，执行调速命令前，可记录下当前的风量大小，以便取消命令设置回原先的速度。

[fn:8] 看到[[Mediator]]的时候再理解这段话。

[fn:9] 实现MapReduce中"Map"的一种方式。

[fn:10] 看到[[proxy]]模式时回头再来补充。

[fn:11] 待学完组合模式再补充完

[fn:12] 具体代码示例参照《Gof设计模式》

[fn:13] 看完[[proxy]]模式再来理解。
