...
** hashable key
#+BEGIN_VERSE
The type should implements both the __hash__ magic
function and either __eq__ or __cmp__ .
#+END_VERSE
- probing function
  #+BEGIN_SRC python
    # pseudocode
...
* Tools
** Numba
   Numba is a compiler that generates optimized machine code from pure
   Python code using the LLVM compiler infrastructure.
* Parallel
** multiprocessing
** concurrent.future
   process pool
