      ...
      for (;;) {
        incoming.wait()
            ...
            .handle<get_balance>([&](get_balance const &msg) {
              msg.atm_queue.send(stm::balance(balance));
            })
...
int main() {
  stm::bank_machine bank;
  stm::interface_machine interface_hardware;
  stm::atm machine(bank.get_sender(), interface_hardware.get_sender());
  std::thread bank_thread(&stm::bank_machine::run, &bank);
  std::thread if_thread(&stm::interface_machine::run, &interface_hardware);
  std::thread atm_thread(&stm::atm::run, &machine);
  ...
  while (!quit_pressed) {
    ...
    case '8':
    case '9':
      atmqueue.send(stm::digit_pressed(c));
      break;
    case 'b':
      atmqueue.send(stm::balance_pressed());
      break;
    case 'w':
      atmqueue.send(stm::withdraw_pressed(50));
      break;
    case 'c':
      atmqueue.send(stm::cancel_pressed());
      break;
    case 'q':
      quit_pressed = true;
      break;
    case 'i':
      atmqueue.send(stm::card_inserted("acc1234"));
      break;
    }
  }
  bank.done();
