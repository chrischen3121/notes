...
1. The distribution of sample mean is approximately normal distribution
2. The standard deviation of the sample(standard error) means $SD \approx\frac{\sigma}{\sqrt{n}}$
...
** Sample Variance
*** Sample variance of sample
     $$s^2 = \frac{\sum_{i=1}^N(x_i-\mu)^2}{N}$$

*** Sample variance of population
    Because population distribution is steeper than sample distribution,
    Use Bessel's Correction: a better estimate of population
