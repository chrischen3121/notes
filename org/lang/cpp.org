...
** 返回*this 的 const 成员函数
   const 成员函数以引用形式返回的是 *常量引用*
...
** 运行时类型识别
   - typeid
   - dynamic_cast: 常用于将基类转换成派生类并执行派生类非虚函数的操作
* 容器
** 数组
   - 声明
     #+BEGIN_SRC cpp
...
* new & delete
  ...
  - 无法改变 *运算符* new 和 delete 的行为
    - 运算符 new 的操作：1) 调用 operator new 获取内存空间 2) 在内存空间中构造对象
