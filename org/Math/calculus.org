...
先分子分母同乘以 $\frac{1}{x^n}$ n为所出现的x的最高次
$$\lim_{x\to -\infty}\frac{4+\frac{1}{x}}{\frac{1}{x}\sqrt{x^2+2}}$$
因为 $\frac{1}{x}$ 为负，极限变为
$$-|\frac{1}{x}|\sqrt{x^2+2}$$
$$\lim_{x\to -\infty}\frac{4+\frac{1}{x}}{-\sqrt{1+\frac{2}{x^2}}} = -4$$
...
*** 二阶导数推导
1) x=a, f'(a)=0
2) f''(a)>0，则函数在x=a有局部最小值
3) f''(a)<0，则函数在x=a有局部最大值
Easy way to rember: 
$$-\cap - \space +\cup +$$
原理：f''(a)>0 => f'(a)在上升，结合 f'(a) = 0 所以x>a时，f'(x)>0 => f(x)在上升，即a点为极小值
** 二阶导数
二阶导数为了表示f(x)的切线斜率的变化率。
- f''(x)>0 表示递增中，凹口向上
- f''(x)<0 表示递减中，凹口向下
- f''(x)=0 表示正在凹口拐点，当然f''(x)经常不存在
#+ATTR_HTML: align="center"
[[file:../resources/calculus/SecondDerivativeMeaning.png]]

** 隐函数微分
$$y^2+xy+3x = 9$$ 
y可以写成关于x的函数，有时不太容易变成y=f(x)的形式，则把y当成关于x的隐函数
...
该版本的意思是，将b-a分割成不等长的n段，即不能使用 $\Delta x$ 。
#+END_VERSE
** 基本法则
*** 幂法则
...
- 特别注意
#+BEGIN_VERSE
n = -1时，分母为0，积分无法应用到 $\frac{1}{x}$ 上。
正确的结果： $\int \frac{1}{x}dx=ln|x|+C$
#+END_VERSE
** 曲线间的面积
若对[a, b]区间上的所有x, $f(x) \ge g(x)$
f(x)与g(x)所夹的面积如下：
...
**** 如何选择u, dv？
1) dv必须选能够积分的项
2) 经过步骤一，右边出现积分项 $\int vdu$ ，如果这个积分应该当比原积分容易解。如果不是，重新选择u, dv。
*** 三角代换法
...
*** 当被积函数变为 $\pm\infty$
$\int_{-1}^2 \frac{1}{x^{\frac{2}{3}}}dx$ 在x=0处未被定义。解法：分段
...
4) $\ln x$ 只对 $x>0$ 有定义
5) $\ln x$ 永远是在递增
6) 由于 $\ln x$ 是 $e^x$ 的反函数，两者关于y=x互为镜像。
* 极坐标
** 画图方法
*** 描点法
*** 转换成直角坐标
...
$$ r= a(1\pm{cos\theta})$$
a代表心脏的大小。正负号代表心脏的凹进部分方向。sin/cos代表是横着的还是竖着的。
** 求面积
* 一些定理
** 介质定理
#+BEGIN_VERSE
若在区间[a, b]上有一连续的函数f, 且p是介于f(a)跟f(b)之间的
任何一个函数值，则在该区间上必定存在一个数c使f(c)=p
Example:
f(x) = 2x 问f(x)在[-1, 1]的区间上是否可能为0
f(x)连续且在区间[-1, 1]的f(x)为[-2, 2]，0在该区间内。
所以必定有一个函数值为0(注意我们在这不太关心函数值为0时x的取值)
#+END_VERSE
** 中值定理(MVT)
- 罗尔定理
#+BEGIN_VERSE
若函数在[a, b]上连续且可微，且若f(a)=0, f(b)=0，则在
区间[a, b]内必须存在一点c, 使得f'(c)=0。
另一种描述：在函数f(x)的曲线上必有一条切线，与该曲线
两端点的连线平行。
#+END_VERSE
- 中值定理
#+BEGIN_VERSE
中值定理只是罗尔定理转了个角度。
定义：若函数f(x)在区间[a, b]区间上连续且可微，则在该区
间内必定存在一点c，使 $f'(c)=\frac{f(b)-f(a)}{b-a}$
#+END_VERSE
