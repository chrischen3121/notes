...
** Hashable Type
   ...
   using the object's placement in memory.(given by ~id~ function)
...
*** Entropy of a Hash Function
    ...
    #+END_SRC
    - Knowing up front *what range of values will be used* and *how large the dictionary will be*
    helps in making a good selection
...
** Dictionary
   ...
   - How well distributed the data is throughout the hash table is called the
   *load factor* and is related to the *entropy* of the hash function
   - By default, the smallest size of a dictionary or set is 8, and it will resize by 3×
   if the dictionary is more than two-thirds full. (possible sizes: 8->18->39->81->165->...)
