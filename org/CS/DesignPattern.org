...
- 创建产品。
  ...
  基于原型的好处：不是每个新的产品系列都需要一个新的具体工厂类。[fn:5]
  #+END_VERSE
...
- [[Mediator]]模式与Facade模式的相似之处：都抽象了一些已有的类的功能。但[[Mediator]]的目的是对同级之间的任意通讯进行抽象。[fn:6]
...
[fn:6] 看到[[Mediator]]的时候再理解这段话。

[fn:7] 实现MapReduce中"Map"的一种方式。

[fn:8] 具体代码示例参照《Gof设计模式》5.4 10
